...
def merge_datasets(data: pd.DataFrame | list[pd.DataFrame], on: str | list[str], suffixes=('_x', '_y')) -> pd.DataFrame:
    """
    ...
    df_merged = reduce(lambda df1,df2: pd.merge(df1 ,df2, on=on, suffixes=('', '_')), data)
    return df_merged
...
def data_distribution(data: list[DataFrame], columns: list[str]):
    ...
    df = reduce(lambda df1,df2: pd.merge(df1 ,df2, on='data', suffixes=('', '_')), data)
